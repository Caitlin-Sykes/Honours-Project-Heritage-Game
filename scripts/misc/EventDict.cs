...
public partial class EventDict : Node2D
{
    ...
    {
        EventLookup.Add("Introduction_Scene", new string[] {"5"});
        ...
        EventLookup.Add("Stonewall_Dialogue", new string[] { "12" });
    }
    ...
    public bool CheckIfSceneTrigger(string name, string trigger) {
        ...
        return EventLookup.ContainsKey(name) && CheckIfTriggerInArray(trigger, name);
    }

    // A function to check if the trigger is present in the dict
    private bool CheckIfTriggerInArray(string trigger, string name) {
